...
namespace bluelotus360.com.razorComponents.MB.Shared.Components.Popups.Transaction
{
    ...
    public partial class GenericReciept
    {
            ...
            [Parameter] public string ReceiptSavingURL { get; set; }
            ...
            {
                if (Transaction != null && Transaction.Location.CodeKey > 10 && Transaction.TransactionKey > 10)
                {
            ...
            public async void OnSaveButtonClick()
            {
                ...
                validateModel = await _transactionManager.SaveAccountRecieptPayementEx(payementModeReciept, ReceiptSavingURL);

                if (!validateModel.isLock)
                {
                    await ReadRecieptsDetails();
