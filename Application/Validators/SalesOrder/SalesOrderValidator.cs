...
namespace BlueLotus360.CleanArchitecture.Application.Validators.SalesOrder
{
    public class SalesOrderValidator : IOrderValidator
    {
        ...
        public bool CanAddItemToGridGeneric()
        {
            UserMessages.UserMessages.Clear();

            //if (_order.SelectedOrderItem.NeedToRequestFromAnotherLocation())
            //{
            //    UserMessages.AddErrorMessage("There is no enough quantity");
            //}
    ...
    public class SalesOrderValidatorV2 : IOrderValidator
    {
        private Order _order;
        public UserMessageManager UserMessages { get; set; }
        public IList<string> RequiredElements { get; set; }
        public SalesOrderValidatorV2(BLUIElement uielement, Order order)
        {
            _order = order;
        ...
        public bool CanSelectItem()
        {
            ...
            if (RequiredElements.Contains("Location") && !BaseResponse.IsValidData(_order.OrderLocation))
            {
                UserMessages.AddErrorMessage("Please Select Order Location");
            }

            if (RequiredElements.Contains("AddressCombo") && !BaseResponse.IsValidData(_order.OrderCustomer))
            {
                UserMessages.AddErrorMessage("Please Select Customer");
            }

            if (RequiredElements.Contains("RepCombo") && !BaseResponse.IsValidData(_order.OrderRepAddress))
            {
                UserMessages.AddErrorMessage("Please Select Rep Address");
            }
            if (RequiredElements.Contains("PaymentTerm") && !BaseResponse.IsValidData(_order.OrderPaymentTerm))
            {
                UserMessages.AddErrorMessage("Please Select Payment mode");
            }
            if (RequiredElements.Contains("Route") && !BaseResponse.IsValidData(_order.AddressCategory3))
            {
                UserMessages.AddErrorMessage("Please Select Route");
            }
            //if (RequiredElements.Contains("PriceList") && !BaseResponse.IsValidData(_order.OrderPaymentTerm))
            //{
            //    UserMessages.AddErrorMessage("Please Select Payment mode");
            //}
        ...
        public bool CanAddItemToGrid()
        {
            ...
            if (!BaseResponse.IsValidData(_order.SelectedOrderItem.TransactionUnit))
            {
                UserMessages.AddErrorMessage("Transaction Unit Cannot be empty");
            }
            if (RequiredElements.Contains("Discount") && _order.SelectedOrderItem.DiscountPercentage == 0)
            {
                UserMessages.AddErrorMessage("Transaction Discount Percentage is 0,can't proceed");
            }
            if (RequiredElements.Contains("VAT") && _order.SelectedOrderItem.ItemTaxType1Per == 0)
            {
                UserMessages.AddErrorMessage("Transaction Vat Percentage is 0 ,can't proceed");
            }
            if (RequiredElements.Contains("SVAT") && _order.SelectedOrderItem.ItemTaxType4Per == 0)
            {
        ...
        public bool CanOrderSave()
        {
            ...
            if (!BaseResponse.IsValidData(_order.OrderLocation))
            {
                UserMessages.AddErrorMessage("Please Select Order Location");
            }

            if (!BaseResponse.IsValidData(_order.OrderCustomer))
            {
                UserMessages.AddErrorMessage("Please Select Customer");
            }

            if (!BaseResponse.IsValidData(_order.OrderRepAddress))
            {
                UserMessages.AddErrorMessage("Please Select Rep Address");
            }
            if (_order.OrderItems.Where(x => x.IsActive == 1).Count() <= 0)
