...
namespace BlueLotus360.CleanArchitecture.Application.Validators.InventoryManagement
{
    public class ItemTransferValidator: IItemTransferValidator
    {
        ...
        public bool CanApproveByScan()
        {
            if (_itemTransfer.SelectedLineItem != null && _itemTransfer.SelectedLineItem.isApr == 1)
            {
                UserMessages.AddErrorMessage("This item is already approved!");
            }
            if (_itemTransfer.SelectedLineItem == null || _itemTransfer.SelectedLineItem.ToItemTrnKey == 1)
            {
                UserMessages.AddErrorMessage("No item can be found!");
            }
