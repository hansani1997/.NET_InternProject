...
namespace BlueLotus360.CleanArchitecture.Application.Validators.HR
{
    ...
    public class HRValidatorExtended : IHRValidator
    {
        private ManualAttendence _time;
        public IList<string> RequiredElements { get; set; }
        public UserMessageManager UserMessages { get; set; }

        public HRValidatorExtended(BLUIElement form, ManualAttendence time)
        {
            UserMessages = new UserMessageManager();
            RequiredElements = form.IsMustElements;
            _time = time;
        }

        public HRValidatorExtended()
        {
            UserMessages = new UserMessageManager();
        }
        ...
        public bool CanAddTimeToGrid()
        {
            ...
            if (RequiredElements.Contains("HeadrSection_RouteCmb") && !BaseResponse.IsValidData(_time.AddressCategory3))
            {
                UserMessages.AddErrorMessage("Please add Route");
            }
            if (RequiredElements.Contains("HeaderSection_Location") && !BaseResponse.IsValidData(_time.Address))
            {
                UserMessages.AddErrorMessage("Please add Location");
            }
            if (RequiredElements.Contains("HeaderSection_Status") && !BaseResponse.IsValidData(_time.WorkReason))
            {
                UserMessages.AddErrorMessage("Please add Shop Status");
            }
